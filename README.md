...
## Setup
 ...
 `docker run -d --cap-add NET_ADMIN --net=host babim/softether:single`
 
`docker run -d --cap-add NET_ADMIN -p 500:500/udp -p 4500:4500/udp -p 1701:1701/tcp -p 1194:1194/udp babim/softether:single`
...
`docker run -d --cap-add NET_ADMIN -p 1194:1194/udp babim/softether:single`
...
`docker run --rm babim/softether:single gencert > /path/to/envlist`
